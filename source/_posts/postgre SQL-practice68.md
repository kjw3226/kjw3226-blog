---
title: postgre SQL-practice68
tag: postgre SQL-practice68
date: 2022-04-20
categories:	SQL
---
...
'7. 검색 결과의 타당성을 지표화하기'로 넘어갔으며

페이지 493 ~ 497를 참고하였다.


검색 결과 상위 n개의 정확률을 계산하는 쿼리문
```python
WITH
...
검색 결과 상위 5개의 정확률을 키워드별로 추출하는 쿼리문
```python
WITH
...
, precision_over_rank_5 AS (
  ...
  FROM
   search_result_with_precision 
  WHERE
   -- 검색 결과의 상위 5개 이하 또는 검색 결과에 포함되지 않은 아이템만 출력하기
   COALESCE(rank, 0) <= 5
)
SELECT
   keyword
...
입니다. 이 지표는 웹 검색 등 방대한 데이터 검색에 적절한 검색 결과를

빠르게 찾고 싶은 경우에 굉장히 중요한 지표라고 할 수 있습니다.

(데이터 분석을 위한 SQL 레시피-8장 데이터를

무기로 삼기 위한 분석 기술 21강 검색 기능 평가하기 497 中)
